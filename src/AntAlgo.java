...
public class AntAlgo implements Runnable{
	...
	BoardFrame window;	//the frame to be painted to see
	boolean stillFood;	//there is still food to grab
	...
	public void run() {
		...
		//will keep going to get food until there is no more food left to grab
		while(stillFood) {
			...
			if(grid[rowPos][colPos].food_count > 0) {
				grid[rowPos][colPos].food_count--;
				hasFood = true;
				prevRow = rowPos;
				prevCol = colPos;
			}
			if(grid[rowLen-1][colLen-1].food_count == 0) {
				stillFood = false;
			}else { 
			
				//adds some scent to the path that the ant took
				if(hasFood) {
					grid[rowPos][colPos].fScent += 5;
				}else {
					grid[rowPos][colPos].nScent += 10;
				}
			...
			window.repaint();	//repaints the new window
			...
			System.out.println(grid[rowLen-1][colLen-1].food_count);
		}
